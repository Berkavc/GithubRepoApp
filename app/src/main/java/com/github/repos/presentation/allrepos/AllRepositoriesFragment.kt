...
class AllRepositoriesFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding = FragmentAllRepositoriesBinding.inflate(
        ...
        allRepoListViewModel.allRepositories.observe(viewLifecycleOwner) {
            when (it) {
                ...
                is ResponseState.Success -> {
                    allRepoList = it.data
                    binding.rvAllRepos.addItemDecoration(
                        ReposItemDecoration(
                            requireContext(),
                    ...
                    allReposAdapter =
                        AllRepositoriesAdapter(object : AllRepositoriesAdapter.ItemClickListener {
                            override fun onClick(username: String, repoName: String) {
                                val fieldBundle =
                                    bundleOf("user_name" to username, "repo_name" to repoName)
                                view.let {
                                    navController.navigate(
                                        R.id.action_fragmentRepos_to_repositoryDetailFragment,
                                        fieldBundle
                                    )
                                }
                            }

                        }, it.data as ArrayList<AllRepositories>)
                    binding.rvAllRepos.adapter = allReposAdapter
                }

                is ResponseState.Error -> {
                    Toast.makeText(
                        requireContext(),
                        it.message ?: R.string.connection_error.toString(),
                        Toast.LENGTH_LONG
                    ).show()
                }
            }
